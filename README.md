...
Troque a URL pela sua e inicie o bot pela raiz:

```sh
npm run start:bot
```

Depois que o bot conectar, use `/liveconfig` no Discord. O painel permite configurar:

- calls que o site deve ignorar;
- categorias inteiras que o site deve ignorar, inclusive seus canais temporários;
- cargos exigidos para transmitir tela ou câmera;
- calls e categorias que dispensam o cargo configurado.

Canais e categorias ignoradas têm prioridade. Sem nenhuma regra cadastrada, todos continuam podendo transmitir. As escolhas são persistidas em `bot/data/live.db` e aplicadas ao site imediatamente; não é preciso reiniciar o bot. Somente membros com **Gerenciar Servidor** podem abrir e alterar o painel.
